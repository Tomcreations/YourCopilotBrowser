...
µb.loadSelectedFilterLists = async function() {
    ...
    const selectedListKeys = new Set(
        hasSavedSelections
            ? bin.selectedFilterLists
            : this.autoSelectRegionalFilterLists(lists)
    );

    // YCB ships uBO as the built-in blocker, so seed a stronger default
    // configuration than the minimal first-run selection. This gives fresh
    // installs immediate coverage for common ad, tracker, privacy, and
    // cosmetic blockers without requiring the user to open the dashboard.
    ...
    for ( const key of strongDefaults ) {
        if ( Object.hasOwn(lists, key) === false ) { continue; }
        selectedListKeys.add(key);
    }
    selectedListKeys.add(this.userFiltersPath);

    const nextListKeys = Array.from(selectedListKeys);
    if (hasSavedSelections) {
        if (nextListKeys.join() === bin.selectedFilterLists.join()) {
            this.selectedFilterLists = bin.selectedFilterLists;
            return;
        }
        this.saveSelectedFilterLists(nextListKeys);
        return;
    }
