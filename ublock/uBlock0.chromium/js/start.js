...
const YCB_D3HOST_ADBLOCK_URL =
    'https://raw.githubusercontent.com/d3ward/toolz/master/src/d3host.adblock';
const onUserSettingsReady = async fetched => {
    ...
    let needsSave = false;
    const importedListSet = new Set(fetched.importedLists);
    if ( importedListSet.has(YCB_D3HOST_ADBLOCK_URL) === false ) {
        importedListSet.add(YCB_D3HOST_ADBLOCK_URL);
        fetched.importedLists = Array.from(importedListSet);
        needsSave = true;
    }
...
try {
    ...
    await Promise.all([
        ...
        µb.loadUserSettings().then(async fetched => {
            ubolog(`User settings ready ${Date.now()-vAPI.T0} ms after launch`);
            await onUserSettingsReady(fetched);
        }),
